...
class DatasetPluginHelper(object):
    ...
    def getTextDataset(self, name):
        ...
        raise DatasetPluginException(
            _("Dataset '%s' is not a text datset") % name)
...
class DatasetPluginManager(object):
    ...
    def saveToFile(self, fileobj):
        ...
        # Save all related dataset attributes
        for ds, dsname in czip(self.veuszdatasets, self.datasetnames):
            dsname1 = names.get(dsname, dsname)
            # skip - deleted
            if dsname1==None:
                continue
            ds.saveDataAttr(fileobj, dsname)
...
def combineMultipliedErrors(inds, length, data):
    ...
    if serr is not None:
        serr = N.abs(N.sqrt(serr) * data)
    if perr is not None:
        perr = N.abs(N.sqrt(perr) * data)
    if nerr is not None:
        nerr = -N.abs(N.sqrt(nerr) * data)
    return serr, perr, nerr
...
class ConcatenateDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        if len(dsin) == 0:
            raise DatasetPluginException(
                _('Requires one or more input datasets'))
...
class ChopDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt(
                'num', _('Maximum number of datapoints'), default=1),
            field.FieldDataset('ds_out', _('Output dataset name')),
        ]
    ...
    def updateDatasets(self, fields, helper):
        ...
        if serr is not None:
            serr = serr[start - 1:start - 1 + num]
        if perr is not None:
            perr = perr[start - 1:start - 1 + num]
        if nerr is not None:
            nerr = nerr[start - 1:start - 1 + num]
...
class PartsDatasetPlugin(DatasetPlugin):
    ...
    def getDatasets(self, fields):
        ...
        if len(self.dsout) == 0:
            raise DatasetPluginException(
                _('Needs at least one output dataset'))
    ...
    def updateDatasets(self, fields, helper):
        ...
        for i, ds in enumerate(self.dsout):
            ...
            if serr is not None:
                pserr = serr[minv:maxv]
            if perr is not None:
                pperr = perr[minv:maxv]
            if nerr is not None:
                pnerr = nerr[minv:maxv]
...
class ThinDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt('start', _('Starting index (from 1)'), default=1),
            field.FieldInt(
                'interval', _('Interval between data points'), default=1),
            field.FieldDataset('ds_out', _('Output dataset name')),
        ]
    ...
    def updateDatasets(self, fields, helper):
        ...
        if serr is not None:
            serr = serr[start - 1::interval]
        if perr is not None:
            perr = perr[start - 1::interval]
        if nerr is not None:
            nerr = nerr[start - 1::interval]
    ...
    def updateDatasets(self, fields, helper):
        ...
        if errortype == 'symmetric':
            ...
            if dsin1.serr is not None:
                ...
                if dsin1.perr is not None:
                    perr1 = dsin1.perr[:minlength]
                if dsin1.nerr is not None:
                    nerr1 = dsin1.nerr[:minlength]
            if dsin2.serr is not None:
                perr2 = nerr2 = dsin2.serr[:minlength]
            else:
                if dsin2.perr is not None:
                    perr2 = dsin2.perr[:minlength]
                if dsin2.nerr is not None:
                    nerr2 = dsin2.nerr[:minlength]
...
class DivideDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        # get error bars
        serr, perr, nerr = combineMultipliedErrors(
            [inds1, inds2], length, data)
...
class DivideMaxPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        # divide error bars
        serr = perr = nerr = None
        if inds.serr is not None:
            serr = inds.serr / maxval
        if inds.perr is not None:
            perr = inds.perr / maxval
        if inds.nerr is not None:
            nerr = inds.nerr / maxval
...
class ExtremesDatasetPlugin(DatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldDataset(
                'ds_min', _('Output minimum dataset (optional)')),
            field.FieldDataset(
                'ds_max', _('Output maximum dataset (optional)')),
    ...
    def getDatasets(self, fields):
        ...
        if not dsout:
            raise DatasetPluginException(
                _('Provide at least one output dataset'))
        return dsout
    ...
    def updateDatasets(self, fields, helper):
        ...
        if self.dserror is not None:
            ...
            mean = tot / num
            self.dserror.update(
                data=mean, nerr=minvals - mean, perr=maxvals - mean)
...
class CumulativePlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        data = cumsum(data)
        if serr is not None:
            serr = N.sqrt(cumsum(serr**2))
        if perr is not None:
            perr = N.sqrt(cumsum(perr**2))
        if nerr is not None:
            nerr = -N.sqrt(cumsum(nerr**2))
        self.dsout.update(data=data, serr=serr, perr=perr, nerr=nerr)
...
class DemultiplexPlugin(DatasetPlugin):
    ...
    name = 'Demultiplex'
    description_short = _(
        'Split dataset into multiple datasets element-by-element')
    ...
    def getDatasets(self, fields):
        ...
        if len(names) == 0:
            raise DatasetPluginException(
                _('Requires at least one output dataset'))
...
class LinearInterpolatePlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldBool(
                'edgenan', _('Use nan for values outside x range')),
...
class ReBinXYPlugin(DatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        self.dssout[0].update(
            data=binydata, serr=binyserr, perr=binyperr, nerr=binynerr)
        ...
        if len(self.dssout) == 2:
            ...
            if dsx.serr is not None:
                ...
                if len(dsx.data) > 2:
                    # assume +ve error bar on last point is as big as its -ve
                    # error
...
class SortPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        if ds.serr is not None:
            out['serr'] = ds.serr[:minlen][idxs]
        if ds.perr is not None:
            out['perr'] = ds.perr[:minlen][idxs]
        if ds.nerr is not None:
            out['nerr'] = ds.nerr[:minlen][idxs]
...
class LogPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        logperr = None if uppers is None else N.log(
            uppers) * invlogbase - logdata
        lognerr = None if lowers is None else N.log(
            lowers) * invlogbase - logdata
