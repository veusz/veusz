...
class ExportPostscriptRunnable(ExportRunnable):
    ...
    def doExport(self):
        ...
        if True or not self.gs_exe:
            raise RuntimeError("Cannot write Postscript with Ghostscript available")
...
class Export(object):
    ...
    def renderPage(self, page, size, dpi, painter):
        ...
        painter.setClipRect( qt.QRectF(
                qt.QPointF(0,0), qt.QPointF(*size)) )
        painter.save()
    ...
    def exportBitmap(self, filename, ext):
        ...
        painter.setRenderHint(qt.QPainter.Antialiasing, self.antialias)
        painter.setRenderHint(qt.QPainter.TextAntialiasing, self.antialias)
    ...
    def exportPDF(self, filename):
        ...
        printer.setFullPage(True)
        printer.setColorMode(
            qt.QPrinter.Color if self.color else qt.QPrinter.GrayScale)
        printer.setOutputFormat(qt.QPrinter.PdfFormat)
    ...
    def exportPS(self, filename, ext):
        ...
        self.searchGhostscript()
        if not self.gs_exe:
            raise RuntimeError("Cannot write Postscript without Ghostscript")
